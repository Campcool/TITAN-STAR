...
(async () => {
  ...
  const result = await M.merge({ months: {} }, entries, async f => XLSX.readFile(path.join(dir, f.name), { cellDates: true }), context.RepairParser.parseWorkbook);
  ...
  const summary = ['## Excel 檢查通過', '', '| 月份 | 維修紀錄 |', '|---|---:|',
    ...
    `最新分析月份：${db.sourceImport.latestMonth}`, '', ...result.warnings,
