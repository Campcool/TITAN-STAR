  ...
  function renderCross() {
    ...
    const body = cross.slice(0, 50).map(p => {
      ...
      const cells = models.map(m => {
        ...
        return `<td onclick="App.openMatrixPartDrawer('${escapeAttr(p.name)}')"><span class="matrix-cell" style="background:${color}${Math.round(intensity * 255).toString(16).padStart(2,'0')};color:${COLORS.text}">${v}</span></td>`;
      }).join('');
      return `<tr>
        ...
        ${cells}
        <td style="cursor:pointer" onclick="App.openMatrixPartDrawer('${escapeAttr(p.name)}')" title="${escapeAttr(p.name)} 合計"><strong style="color:var(--text);font-family:var(--mono)">${p.count}</strong></td>
      </tr>`;
    }).join('');
